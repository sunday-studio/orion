...
Public notification fan-out should enqueue from published `status_page_incident_updates` or scheduled maintenance publication events, not from internal incident events. The payload builder must accept only public DTOs so it cannot accidentally include agent ids, monitor ids, private notes, raw reports, internal hostnames, or alert channel internals.
