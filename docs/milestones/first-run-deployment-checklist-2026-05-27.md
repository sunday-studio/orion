...
This pass exercised a real local self-hosted Core deployment in Docker Compose plus a native Agent binary on the host. It used a temporary Core image and isolated Docker Compose project so the deployment path, registration, reporting, restart behavior, logs, and backup flow could be verified without touching installed system services.

This is concrete smoke evidence, but it is not the full first-run checklist closeout. The installed Agent service and Console UI checks still need to be run on a host where service installation and interactive Console verification are available.
...
- Agent binary: `/private/tmp/orion-agent-first-run`
- Agent version: `first-run-checklist`
- Agent config: `tmp/first-run-agent-config.yaml`
- Agent state: `/private/tmp/orion-first-run-state.db`
- Agent log: `/private/tmp/orion-first-run-agent.log`
...
## Agent Registration And Reports

The first Agent run used explicit config and state paths:
...
It registered one Agent and two monitors:
...
Core reported exactly one Agent:
...
Agent report and monitor history evidence:
...
The local Agent state database and log file were present:
...
The same Agent binary was run again with the same config and state path:
...
No duplicate Agent or monitor rows were observed.
...
- `T-20260527-223714-60db`: Verify installed Agent service on first-run host.
- `T-20260527-223711-3fef`: Capture Console first-run verification.

Scope note: the Agent ran as a native host process against a local Docker Core rather than as an installed service on a separate remote household machine.
