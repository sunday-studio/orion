...
func (s *Server) getPublicStatusPageIncidentHistory(c *gin.Context) {
	...
	s.writePublicStatusPageJSON(c, http.StatusOK, "Status page incident history retrieved successfully", gin.H{"history": StatusPagePublicIncidentHistoryResponse{Incident: incident, Updates: updates}})
}
...
func (s *Server) publicStatusPageComponentHistories(detail StatusPageDetailResponse, window string, includeBuckets bool) []StatusPagePublicComponentHistoryResponse {
	...
	for _, component := range detail.Components {
		...
		responses = append(responses, StatusPagePublicComponentHistoryResponse{Component: s.statusPagePublicComponentResponse(component, s.statusPageComponentStatus(component)), Uptime: uptime, History: history})
	}
	return responses
}
...
func (s *Server) publicStatusPageMappedResourceUptime(mapping StatusPageComponentMappingResponse, window string) (statusPagePublicResourceUptime, bool) {
	...
	return statusPagePublicResourceUptime{buckets: result.DailyBuckets, totalUp: result.UpCount, totalSamples: result.TotalCount, uptimePercent: result.UptimePercent}, true
}
...
func publicUptime(window string, ratio *float64) StatusPagePublicUptimeResponse {
	return StatusPagePublicUptimeResponse{Window: window, Status: publicUptimeStatus(ratio), UptimeRatio: publicRoundedRatio(ratio), UptimeDisplay: publicUptimeDisplay(ratio)}
}
