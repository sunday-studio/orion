...
func TestStatusPageAdminAPIFlow(t *testing.T) {
	...
	if err := server.db.Model(&db.Monitor{}).Where("id = ?", registeredMonitor.Data.MonitorID).Updates(map[string]any{"health": "down", "computed_health": "down"}).Error; err != nil {
		t.Fatalf("update monitor health: %v", err)
	}
	createPageResp := performJSONRequest(t, server, http.MethodPost, "/v1/status-pages", gin.H{"slug": "main-status", "title": "Main Status", "description": "Customer-facing availability", "theme_settings": gin.H{"accent_color": "#10b981"}}, "")
	...
	createSectionResp := performJSONRequest(t, server, http.MethodPost, "/v1/status-pages/"+createdPage.Data.Page.ID+"/sections", gin.H{"name": "API", "sort_order": 10, "collapsed_by_default": false}, "")
	...
	updateSectionResp := performJSONRequest(t, server, http.MethodPut, "/v1/status-pages/"+createdPage.Data.Page.ID+"/sections/"+createdSection.Data.Section.ID, gin.H{"name": "Core API", "collapsed_by_default": true}, "")
	...
	createComponentResp := performJSONRequest(t, server, http.MethodPost, "/v1/status-pages/"+createdPage.Data.Page.ID+"/components", gin.H{"section_id": createdSection.Data.Section.ID, "public_name": "REST API", "public_description": "Primary API", "display_mode": "single_resource", "sort_order": 5, "visible": true}, "")
	...
	if createdComponent.Data.Component.ID == "" || createdComponent.Data.Component.SectionID != createdSection.Data.Section.ID || createdComponent.Data.Component.PublicName != "REST API" || !createdComponent.Data.Component.Visible {
		t.Fatalf("created component = %+v, want visible REST API component", createdComponent.Data.Component)
	}
	createMappingResp := performJSONRequest(t, server, http.MethodPost, "/v1/status-pages/"+createdPage.Data.Page.ID+"/components/"+createdComponent.Data.Component.ID+"/mappings", gin.H{"resource_type": "monitor", "resource_id": registeredMonitor.Data.MonitorID, "health_rollup_strategy": "worst", "uptime_rollup_strategy": "worst"}, "")
	...
	if createdMapping.Data.Mapping.ID == "" || createdMapping.Data.Mapping.ResourceType != "monitor" || createdMapping.Data.Mapping.ResourceID != registeredMonitor.Data.MonitorID {
		t.Fatalf("created mapping = %+v, want monitor mapping", createdMapping.Data.Mapping)
	}
	updateMappingResp := performJSONRequest(t, server, http.MethodPut, "/v1/status-pages/"+createdPage.Data.Page.ID+"/components/"+createdComponent.Data.Component.ID+"/mappings/"+createdMapping.Data.Mapping.ID, gin.H{"health_rollup_strategy": "average"}, "")
	...
	createIncidentResp := performJSONRequest(t, server, http.MethodPost, "/v1/status-pages/"+createdPage.Data.Page.ID+"/incidents", gin.H{"title": "Elevated API errors", "public_status": "scheduled", "severity": "high", "impact_summary": "Some requests are failing.", "visibility": "draft", "affected_component_ids": []string{createdComponent.Data.Component.ID}, "scheduled_start_at": scheduledStart, "scheduled_end_at": scheduledEnd}, "")
	...
	if createdIncident.Data.Incident.ID == "" || createdIncident.Data.Incident.Title != "Elevated API errors" || createdIncident.Data.Incident.PublicStatus != "scheduled" || createdIncident.Data.Incident.ScheduledStartAt == "" || createdIncident.Data.Incident.ScheduledEndAt == "" || len(createdIncident.Data.Incident.AffectedComponentIDs) != 1 || createdIncident.Data.Incident.AffectedComponentIDs[0] != createdComponent.Data.Component.ID {
		t.Fatalf("created incident = %+v, want API incident", createdIncident.Data.Incident)
	}
	updateIncidentResp := performJSONRequest(t, server, http.MethodPut, "/v1/status-pages/"+createdPage.Data.Page.ID+"/incidents/"+createdIncident.Data.Incident.ID, gin.H{"public_status": "scheduled", "visibility": "published", "published_at": now, "scheduled_start_at": scheduledStart, "scheduled_end_at": scheduledEnd}, "")
	...
	createUpdateResp := performJSONRequest(t, server, http.MethodPost, "/v1/status-pages/"+createdPage.Data.Page.ID+"/incidents/"+createdIncident.Data.Incident.ID+"/updates", gin.H{"status": "monitoring", "message": "A mitigation is in place.", "created_by": "ops", "published_at": now}, "")
	...
	if detail.Data.Incidents[0].PublicStatus != "monitoring" || len(detail.Data.Incidents[0].Updates) != 1 || detail.Data.Incidents[0].Updates[0].Message != "A mitigation is in place." {
	...
	if preview.Data.Preview.Page.Slug != "main-status" || len(preview.Data.Preview.Sections) != 1 || len(preview.Data.Preview.Sections[0].Components) != 1 || preview.Data.Preview.Sections[0].Components[0].Status != "major_outage" {
	...
	if publicPage.Data.StatusPage.Page.Slug != "main-status" || publicPage.Data.StatusPage.OverallStatus != "major_outage" || len(publicPage.Data.StatusPage.Sections) != 1 || len(publicPage.Data.StatusPage.Sections[0].Components) != 1 || len(publicPage.Data.StatusPage.Incidents) != 1 {
	...
	if componentProjection.Uptime == nil || componentProjection.Uptime.Window != statusPagePublicDefaultUptimeWindow || len(componentProjection.UptimeHistory) != publicWindowDays(statusPagePublicDefaultUptimeWindow) {
	...
	if publicPage.Data.StatusPage.Incidents[0].ScheduledStartAt == "" || publicPage.Data.StatusPage.Incidents[0].ScheduledEndAt == "" {
	...
	for _, path := range []string{"/v1/status-pages", "/v1/status-pages/" + createdPage.Data.Page.ID + "/sections", "/v1/status-pages/" + createdPage.Data.Page.ID + "/components", "/v1/status-pages/" + createdPage.Data.Page.ID + "/components/" + createdComponent.Data.Component.ID + "/mappings", "/v1/status-pages/" + createdPage.Data.Page.ID + "/incidents"} {
