...
func (s *Server) createStatusPageSection(c *gin.Context) {
	...
	section := db.StatusPageSection{ID: utils.GenerateID("status_page_section"), StatusPageID: c.Param("id")}
...
func (s *Server) createStatusPageComponent(c *gin.Context) {
	...
	component := db.StatusPageComponent{ID: utils.GenerateID("status_page_component"), StatusPageID: c.Param("id"), DisplayMode: "single_resource", Visible: true}
...
func (s *Server) createStatusPageComponentMapping(c *gin.Context) {
	...
	mapping := db.StatusPageComponentMapping{ID: utils.GenerateID("status_page_mapping"), ComponentID: component.ID, HealthRollupStrategy: "worst", UptimeRollupStrategy: "worst"}
	...
	if err := s.db.Transaction(func(tx *gorm.DB) error {
		...
		return s.recordStatusPageAuditEvent(tx, c, service.StatusPageAuditEventInput{Action: service.StatusPageAuditActionComponentMappingCreated, StatusPageID: c.Param("id"), AffectedObjectType: "component_mapping", AffectedObjectID: mapping.ID})
	}); err != nil {
...
func (s *Server) updateStatusPageComponentMapping(c *gin.Context) {
	...
	if err := s.db.Transaction(func(tx *gorm.DB) error {
		...
		return s.recordStatusPageAuditEvent(tx, c, service.StatusPageAuditEventInput{Action: service.StatusPageAuditActionComponentMappingUpdated, StatusPageID: c.Param("id"), AffectedObjectType: "component_mapping", AffectedObjectID: mapping.ID})
	}); err != nil {
		writeStatusPageCreateError(c, err, "Status page component mapping already exists")
		return
	}
	utils.SuccessResponse(c, http.StatusOK, "Status page component mapping updated successfully", gin.H{"mapping": statusPageComponentMappingResponse(mapping)})
}
