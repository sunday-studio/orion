...
func (s *Server) createStatusPageIncidentDraft(c *gin.Context) {
	...
	publicIncident := db.StatusPageIncident{ID: utils.GenerateID("status_page_incident"), StatusPageID: pageID, InternalIncidentID: internalIncident.ID, Title: draft.Title, PublicStatus: draft.PublicStatus, Severity: draft.Severity, ImpactSummary: draft.ImpactSummary, Visibility: statusPageIncidentVisibilityDraft, AffectedComponentIDs: encodeStringList(draft.AffectedComponentIDs)}
	update := db.StatusPageIncidentUpdate{ID: utils.GenerateID("status_page_incident_update"), IncidentID: publicIncident.ID, Status: draft.PublicStatus, Message: draft.InitialUpdateMessage, CreatedBy: "orion"}
	if err := s.db.Transaction(func(tx *gorm.DB) error {
		...
		if err := s.recordStatusPageAuditEvent(tx, c, service.StatusPageAuditEventInput{Action: service.StatusPageAuditActionPublicIncidentCreated, StatusPageID: publicIncident.StatusPageID, AffectedObjectType: "public_incident", AffectedObjectID: publicIncident.ID}); err != nil {
			return err
		}
		return s.recordStatusPageAuditEvent(tx, c, service.StatusPageAuditEventInput{Action: service.StatusPageAuditActionPublicIncidentUpdateCreated, StatusPageID: publicIncident.StatusPageID, AffectedObjectType: "public_incident_update", AffectedObjectID: update.ID})
	}); err != nil {
	...
	utils.SuccessResponse(c, http.StatusCreated, "Status page incident draft created successfully", gin.H{"draft": draft, "incident": statusPageIncidentResponse(publicIncident, []StatusPageIncidentUpdateResponse{statusPageIncidentUpdateResponse(update)}), "update": statusPageIncidentUpdateResponse(update)})
}
