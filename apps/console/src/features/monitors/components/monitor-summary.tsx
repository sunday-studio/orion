import type { ServiceMonitorSummary } from "@/orion-sdk";
import { cn } from "@/lib/utils";

export type MonitorSummaryFilter =
  | "all"
  | "up"
  | "down"
  | "degraded"
...
type MonitorSummaryProps = {
  selectedFilter: MonitorSummaryFilter;
  summary?: ServiceMonitorSummary;
  onFilterChange: (filter: MonitorSummaryFilter) => void;
};
...
export const MonitorSummary = ({
  ...
  const label = total === 1 ? "monitor" : "monitors";
  const items: Array<{
    filter: MonitorSummaryFilter;
    label: string;
    value: string | number;
  ...
  return (
    <div className="grid gap-1 py-2 text-sm sm:grid-cols-4 xl:grid-cols-7">
      {items.map((item) => {
        ...
        return (
          ...
          >
            ...
            <span
              className={cn(
                "text-lg font-medium text-neutral-600",
                isSelected && item.selectedTextClassName,
              )}
            >
              {item.value}
