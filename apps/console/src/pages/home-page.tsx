...
export function HomePage() {
  ...
  const expandedMonitors =
    dataOf<ListMonitorsResponseData>(monitorsRes)?.monitors ?? [];
          ...
          {view === "list" && (
            <>
              ...
              <select
                value={status}
                onChange={(e) => {
                  setStatus((e.target.value || "") as "" | ListAgentsStatus);
                  setOffset(0);
                }}
                  ...
                  {agents.map((a) => (
                    <Fragment key={a.id}>
                      <tr>
                        ...
                        <td>
                          {a.id ? (
                            <Link to={`/agents/${a.id}`}>{a.name ?? a.id}</Link>
                          ) : (
                            (a.name ?? "—")
                          )}
                        </td>
                        <td>
                          {a.ip ??
                            (a.location as { ip?: string } | undefined)?.ip ??
                            "—"}
                        </td>
                        ...
                        <td>
                          {a.monitor_count != null ? a.monitor_count : "—"}
                        </td>
                      </tr>
                      {expanded === a.id && (
                        <tr>
                          <td colSpan={8} className="expanded">
                            <MonitorsSubTable
                              list={expanded === a.id ? expandedMonitors : []}
      ...
      <tbody>
        {list.map((m) => (
          <tr key={m.id}>
            <td>
              {m.id ? (
                <Link to={`/monitors/${m.id}`}>{m.name ?? m.id}</Link>
              ) : (
                (m.name ?? "—")
              )}
            </td>
