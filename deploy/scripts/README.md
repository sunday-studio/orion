...
3. **Create** `/etc/orion/config.yaml` (Linux) or `/usr/local/etc/orion/config.yaml` (macOS) with `core_url`, `interval`, and optional `monitors`. Create `/var/lib/orion` (Linux) or `/usr/local/var/lib/orion` (macOS) for `state.db`.
